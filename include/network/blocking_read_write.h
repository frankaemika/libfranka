...
namespace franka {
/**
 * BlockingReadWrite implements blocking read and write calls. This class does
 * not handle
...
class BlockingReadWrite {
 public:
  BlockingReadWrite(std::shared_ptr<boost::asio::io_service> service,
                    std::shared_ptr<Socket> socket)
      : io_service_{service}, socket_{socket} {}
  ...
  std::vector<unsigned char> receive(size_t size,
    ...
    // TODO: apply timeout for the whole operation, not just the single receive
    // call
 ...
 private:
  std::size_t receive(unsigned char* data,
                      size_t size,
                      boost::posix_time::time_duration timeout_duration) {
    boost::asio::deadline_timer timer(*io_service_, timeout_duration);
    bool timeout_fired = false;
