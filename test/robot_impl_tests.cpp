...
TEST(RobotImpl, ThrowsTimeoutIfNoRobotStateArrives) {
  ...
  MockServer server;
  Robot::Impl robot(std::make_unique<franka::RobotNetwork>("127.0.0.1", kCommandPort, 200ms));
...
TEST(RobotImpl, StopsIfControlConnectionClosed) {
  std::unique_ptr<Robot::Impl> robot;
  {
    ...
    robot.reset(
        new Robot::Impl(std::make_unique<franka::RobotNetwork>("127.0.0.1", kCommandPort, 200ms)));
