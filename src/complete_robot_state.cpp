...
CompleteRobotState& CompleteRobotState::operator=(
    const research_interface::RobotState& robot_state) noexcept {
  ...
  std::copy(robot_state.dtau_J.cbegin(), robot_state.dtau_J.cend(),
            dtau_J.begin());
  std::copy(robot_state.q.cbegin(), robot_state.q.cend(), q.begin());
  std::copy(robot_state.dq.cbegin(), robot_state.dq.cend(), dq.begin());
  std::copy(robot_state.q_d.cbegin(), robot_state.q_d.cend(), q_d.begin());
  std::copy(robot_state.joint_contact.cbegin(),
            robot_state.joint_contact.cend(), joint_contact.begin());
  std::copy(robot_state.cartesian_contact.cbegin(),
            robot_state.cartesian_contact.cend(), cartesian_contact.begin());
  std::copy(robot_state.joint_collision.cbegin(),
            robot_state.joint_collision.cend(), joint_collision.begin());
  ...
  std::copy(robot_state.O_F_ext_hat_K.cbegin(),
            robot_state.O_F_ext_hat_K.cend(), O_F_ext_hat_K.begin());
  std::copy(robot_state.K_F_ext_hat_K.cbegin(),
            robot_state.K_F_ext_hat_K.cend(), K_F_ext_hat_K.begin());
  return *this;
}
