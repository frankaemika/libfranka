...
bool Robot::Impl::update() {
    ...
    try {
        ...
        udp_socket_.sendBytes(*robot_command_, sizeof(robot_command_));
            ...
            throw NetworkException("libfranka: robot command write: "s + e.what());
    }
    */
}
...
const CartesianPoseMotionGenerator& Robot::Impl::startCartesianPoseMotionGenerator() {
    if (!motion_generator_running_) {
        ...
        if (setCartesianPoseMotionGeneratorInRCU()) {
            ...
            throw MotionGeneratorException("libfranka:: Could not enable motion generator!");
        }
    } else {
        throw MotionGeneratorException("libfranka:: Attempt to start multiple motion generators!");
    }
}
